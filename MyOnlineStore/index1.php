...
if (!isset($_SESSION["cart_array"]) || count($_SESSION["cart_array"]) < 1) {
    ...
    foreach ($_SESSION["cart_array"] as $each_item) { 
		...
		$cartOutput .= '<div class="shopp" id="each-'. $item_id .'"><div class="label">'. $product_name .'</div><div class="shopp-price"> $<em>'. $price .'</em></div><div class="shopp-quantity"><form action="index.php" method="post">
		<input name="quantity" type="text" value="' . $each_item['quantity'] . '" size="1" maxlength="2" />
		<input name="adjustBtn' . $item_id . '" type="submit" value="change" />
		<input name="item_to_adjust" type="hidden" value="' . $item_id . '" />
		</form></div></em><div class="pricetotal">' . $pricetotal .      '</div><div class="remove"><form action="index1.php?id='.$pg.'" method="post"><input type="image" name="deleteBtn' . $item_id . '" src="remove.png" /><input name="index_to_remove" type="hidden" value="' . $i . '" /></form></div></div>';
		$i++; 
    } 
	setlocale(LC_MONETARY, "en_US");
    $cartTotal = money_format("%10.2n", $cartTotal);
	$cartTotal = "<b>Cart Total</b> : ".$cartTotal." Rs";
...
if (isset($_GET['id'])) {
	// Connect to the MySQL database  
    include "storescripts/connect_to_mysql.php"; 
	$ccid = preg_replace('#[^0-9]#i', '', $_GET['id']); 
$mysql = mysql_query("SELECT category FROM category where id='$ccid'");
$row1 = mysql_fetch_array($mysql);
$cat = $row1["category"];
$category=$cat;
$dynamicList = "";
$sql = mysql_query("SELECT * FROM products where subcategory='$category' ORDER BY date_added  DESC LIMIT 6");
...
<div align="center" id="mainWrapper">
  ...
  <div id="pageContent">
   ...
   <p>welcome <?php echo $loginuser; ?></p>
